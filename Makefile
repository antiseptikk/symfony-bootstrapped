...
db-reset: .env vendor
	...
	@$(EXEC_PHP) php -r 'echo "Wait database...\n"; set_time_limit(15); var_dump(__DIR__); require __DIR__."/vendor/autoload.php"; (new \Symfony\Component\Dotenv\Dotenv())->load(__DIR__."/.env"); $$u = parse_url(getenv("DATABASE_URL")); for(;;) { if(@fsockopen($$u["host"].":".($$u["port"] ?? 3306))) { break; }}'
